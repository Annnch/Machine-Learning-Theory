...
For any $k \neq j$ it holds:
\begin{align*}
\frac{\sum_{\xi: \xi_t = j, \xi_{t + 1} = j} w_t^\xi}{\sum_{\xi: \xi_t = j, \xi_{t + 1} = k} w_t^\xi} & = \frac{\sum_{\xi: \xi_t = j, \xi_{t + 1} = j} \frac{1}{K}\prod * (1 - \alpha) \prod * e^{-\sum *}}{\sum_{\xi: \xi_t = j, \xi_{t + 1} = j} \frac{1}{K}\prod * \frac{\alpha}{K - 1} \prod * e^{-\sum *}} \\
