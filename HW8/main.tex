...
\[
\min_{w \in \Delta_K} w^T l_t + \text{KL}(w, w_t) = w_{t + 1}; \quad w_{t+1}^k = \frac{w_t^k e^{-l_t^k}}{\sum_{j = 1}^K w_t^j e^{-l_t^j}}
\]
...
& = \min_{w \in \Delta_K} \sum_{k = 1}^K w^k \left( l_1^k + \ln K w^k \right) \\
& = \min_{w \in \Delta_K} e^{\sum_{k = 1}^K w^k \left( l_1^k + \ln K w^k \right)} \\
& = \min_{w \in \Delta_K} \prod_{k = 1}^K e^{w^k \left( l_1^k + \ln K w^k \right)} \\
...
\begin{align*}
w^T l_t + \text{KL}(w, w_t) & = w^Tl_t + \sum_{k = 1}^K w^k \ln \frac{w^k}{w_t^k} \\
